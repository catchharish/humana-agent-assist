...
**Revision:** September 17, 2026 — Version A base, with selected Version B changes and the subsequent review corrections incorporated.  
**Working proposition:** Help advocates prevent avoidable compliance errors and give consistent, member-specific help during the call, then produce an accurate wrap and handoff.

This is one integrated contract, not an addendum. Use it with the original Humana brief. Earlier A/B definitions and comparison notes are not additional build instructions. A's section structure, synthetic reference world, scoped member choice, authority boundaries, and outcome meanings are retained except where this document explicitly revises them.
...
| **Product decision** | The chosen behavior in this contract. Reconciled choices are identified in section 31; they are not claims that Humana mandated them. |
...
| **Prior-experience evidence [F]** | The supplied Canonical Fact Base governs personal history; the Story Bank is derivative. Section 27 includes the relevant supported facts and limits. |

Source key for decision provenance: **[A]** `FINAL_PRODUCT_DECISIONS_A(1).md`; **[B]** `FINAL_PRODUCT_DECISIONS_B(1).md`; **[R]** `Pasted markdown(4).md`; **[F]** `Harish_PM_Canonical_Fact_Base_v0_2(1).docx`. [R] is a comparative review, not a source of actual Humana policy. Section references after these labels refer to the original source sections. The final decisions below, rather than the review's verdict, govern implementation.

### Build scope: must build, must prove, and later

This imports B's tiered handoff, **not B's full tier contents**. Sections 18–20 replace A's requirement to run the whole variant catalog repeatedly.

| Tier | Required scope |
|---|---|
| **1 — Must build** | One interactive Harry call on a timed synthetic stream; visible call type/current workflow and open needs; exact wording and evidence-based nudge; sourced member answers; one governed common answer with lineage; one optional next-best-action and one respectful objection response; scoped human-only enrollment; justified live handoff and returned connection status; generated editable wrap and confirmed disposition code. Include basic source inspection, focus correction, safe retry/edit/status recovery, a subordinate governed quick ask, and limited closing-attestation recovery. Log evidence, event-to-display timing, and available usage. |
| **2 — Must prove** | Two fresh end-to-end runs, eight short contrasting replays, and the small direct-check set in section 19. Inspect the interface as well as logs. Use exact checks where possible and disclosed human review for semantic claims. No 100-run mandate, large corpus quota, or calibrated model judge is required. |
| **3 — Defined for later** | The remaining test catalog, broad call coverage, real speech recognition and integrations, representative trust/workload studies, large golden sets, automated judge calibration, automated call-log mining, detailed operating-cost allocation, and cross-call dashboards. These are not take-home acceptance requirements. |
...
**D01 — Compliance-led, whole-call assistance.** Help advocates prevent avoidable compliance errors and give consistent, member-specific service by keeping the current task, applicable wording, relevant evidence, and next step aligned while they talk to the member.
...
**Decision provenance:** A's whole-call and member-choice foundation; B §5's simultaneous-jobs framing and §7's explicit compliance outcome; revised after the business-priority review to match [H]. Neither version's productivity-led headline is retained.
...
**Reduce avoidable compliance failures and deliver more consistent member service during the call, rather than relying mainly on sampled review afterward. Recover advocate capacity by reducing searching, manual tracking, corrections, and after-call reconstruction.**

| Outcome | Why the product could improve it | What would establish improvement |
|---|---|---|
| **Lead: disclosure compliance** | Surface exact wording before the governed deadline; detect a reliable miss or paraphrase promptly; provide actionable recovery. | Consistent independent review of applicable requirements in comparable assisted and baseline calls, with uncertainty and review coverage visible. |
...
**Prevention and recovery are different.** Reading the required wording before its deadline supports compliant handling. Once the deadline is crossed, a nudge can improve detection and recovery but cannot prevent that already-occurring violation. The main demo intentionally includes the second situation. A clean contrasting pricing segment shows the first. Neither proves the size of a real-world causal improvement.

The lead metric is independently reviewed disclosure failures per applicable requirement opportunity, by requirement type. Unverified evidence, unknown applicability, review coverage, service quality, and authority failures remain visible alongside it. Section 24 defines the measures and expansion decision.
...
**Decision provenance:** Replaces A §§7/24 and B §§7/24 productivity-led headline/ratio. Keeps A's population and accounting discipline while treating compliance as an intended improvement in its own right.
...
The presenter plays the advocate for meaningful choices: recommendation acceptance/dismissal, reread or supported attestation, scoped human submission, transfer confirmation, and disposition confirmation. The generated answers, detected states, and drafts must depend on the supplied inputs; the presenter cannot set them to the reference answer. A test driver may repeat human actions for regression.
...
Keep the live missed-disclosure nudge, scoped enrollment boundary, live handoff, and generated wrap in the main demonstration. Shorten routine explanation or source browsing first; omit audio, diagnostics detail, and the optional QA view before cutting a required experience. The detailed regression variants are not additional main-call scenes. Record actual rehearsal length; do not claim an unmeasured duration.
...
**Attestation choice in this final contract:** Import B's separate evidence class, but permit it only for the advocate's own uncertain closing attempt under the explicit synthetic v2 rule. The main call still uses a clear reread. This deliberately changes A's reread-only recovery, not its evidence standard. Actual Humana allowance for attestation is unknown and must be obtained before field use. Do not extend it to the pricing prerequisite, greeting, definite failures, or member consent.
...
**Decision provenance:** A's evidence and context-specific recovery rules; B §12's quick ask, separate attestation class, fixed nudge copy, and degraded-mode treatment, limited as stated here.
...
|---|---|
| **Call type / current workflow** | Unclassified/general inquiry; refill-status servicing; historical-price inquiry; prospective comparison; service education/election; coverage-status/handoff. Recognize and update automatically from reliable conversation and system evidence. Labels describe work; they do not authorize it. |
| **Member needs** | Requested, active/deferred, resolved on evidence, declined/withdrawn, transferred/pending. One call can contain several. |
...
**Decision provenance:** A §13's multi-need/evidence model plus B §§13–14's explicit call-type and flow visibility. B's confirmation-dependent type/obligation registration is not imported.

## 14. UI and attention-management behavior

**D05:** One guided workspace, with one dominant current-task card. B's five-region layout is the reference arrangement; exact pixels and responsive layout remain implementation choices.

| Region | Required experience |
|---|---|
| **Call strip — top** | Identity/role status, current call type, relevant flow step, and elapsed time. Before authorization, no member name, plan, medicines, or other protected facts. After authorization, show only relevant member context. Type/step updates are automatic, with no routine confirmation click. |
...
| **Context drawer — subordinate** | Relevant member facts, other open needs, valid deferred answers, source details/history, and the governed quick-ask input. The advocate can pull context without losing the current call. |
...
**Decision provenance:** A's attention, recovery, accessibility, and trust principles; B's call strip, obligation rail, source-type labels, compact transcript, fixed compliance copy, and visible paraphrase difference.

## 15. Information requirements

**D13 is the authoritative fictional world for the main call and reference variants.** A's fixed records, prices, service effects, and chosen medication scope are retained. The only disclosure-policy change is the explicitly versioned closing recovery below. All names beyond the brief's sample, IDs, quantities, prices, dates, classifications, policies, permissions, service effects, and results are authored demonstration material. They are not real Humana member records, prices, coverage rules, Legal-approved wording, or actual system behavior. A requested medication in a coverage case is not clinical advice or an active prescription.
...
Each record visibly says **'Synthetic demo policy — not actual Humana-approved wording.'** Use DEMO-GREETING-v1, DEMO-PRICING-v1, and DEMO-CLOSING-v2 for this replay and the same applicable versions in post-call QA. Closing v2 keeps A's words and deadline, clarifies member-initiated service discussion, and adds the limited attestation option below; it does not silently overwrite v1.
...
**Scope choice:** The two historical purchases and six prospective quote records stay because they are already specified in A and support one small comparison table. They do not require six screens or integrations. The main-call metformin-only choice is a chosen test of scoped authorization, not a universal requirement to compare or enroll two medications. Do not make retail preference an obstacle to a desired conversion.
...
**Decision provenance:** A §§16–17's evidence-first flexibility; B §§16–17's explicit separation of structured facts, governed knowledge, derived guidance, and later call-log mining.
...
B's three evidence classes organize the methods, not the importance of a failure:
...
| **Core useful behavior** | Required main-call outputs and the eight contrasting behaviors pass their reference assertions. Silent, partial, declined, pending, and unavailable are sometimes the correct outcomes. |
...
| **Interaction / recovery** | The presenter can inspect evidence, correct focus, edit a draft, use quick ask, make meaningful human decisions, and recover without a false result or extra routine approvals. Check the UI as well as logs. |
...
| **T03A — Member-led service inquiry** | New call with exact greeting and authorization; member asks about the 90-day option. No refill or quote. No NBA acceptance click. Advocate explains the optional service and closes. | Automatically show service type/flow and supported guidance. Actual advocate service discussion triggers closing v2 without an NBA click. Greeting and closing apply; pricing does not. No invented refill or enrollment. |
...
| **T07B — Closing attestation, not verification** | Main-call state through its late pricing correction and pending handoff. Closing attempt has explicit uncertainty; no clear reread. Advocate attests before exit under closing v2. | Record ATTESTED / speech not verified and permit the declared manual continuation. No clean verification or definite miss. Expected requirement counts: **1 satisfied, 1 pricing finding, 1 unverified/attested, N=3**. Main-call outcome is not copied. No consent attestation. |
...
| **T09B — Unknown write outcome** | Valid metformin-only human submission times out. A status check for that same request returns PENDING; no final success is supplied. | Show outcome unknown, allow status check rather than resubmission, and keep zero duplicate writes. Wrap states unconfirmed enrollment and retains the requested scope; timeout is neither success nor definite failure. |
...
| **C03 — Access / untrusted data** | No member fields before explicit authorization; returned wrong-member data is rejected; revocation invalidates protected assistance. An instruction inside retrieved content cannot override confirmation, identity, or the human-only boundary. |
| **C04 — Recovery / degraded operation** | Quick ask uses the same sources and boundaries; a read gets at most the declared retry before manual review. Monitoring outage makes the affected interval unverified while exact text stays accessible. No invented buffered speech. Wrap-generation failure yields a labeled manual template. |
...
**Q01:** Recompute T01 and T07B from transcript, rule versions, attestations, and action evidence; obtain their stated counts.  
...
| Closing | Initial DEMO-CLOSING-v2 attempt has uncertain evidence. Main-call advocate gives a new clear exact reading before exit. | Preserve uncertainty history; new timely reading satisfies closing. T07B exercises attestation separately. |
...
| Optional QA | Reconstruct from the actual evidence and applicable registry. | 2/3 satisfied, one timing finding, zero ultimately unverified; action authority is assessed separately. |
...
**Clean pricing contrast:** C01 uses the same quote evidence but a reliable exact statement before the quote. The expected pricing outcome is timely, with no pricing finding. This proves the correct-before-deadline behavior in that synthetic segment; it is not a measured real-world prevention effect or a claim that the same advocate would otherwise violate the rule.
...
These are retained A design targets, not Humana SLAs. Record raw timing, counts, normal misses, no-output cases, human pauses, and injected-fault intervals. Do not count a spinner or unsupported streamed fragment as useful completion. A late answer can still be useful for the active need; do not demote it solely to hide the missed target.
...
| **Finding evidenced** | A condition demonstrably failed. Late correction does not erase it. Retain any other uncertainty in detail. |
...
Show **S / N fully satisfied**, **F findings**, and **U unable to verify**, with S + F + U = N for known-applicable instances. Do not hide uncertain instances from N. Unknown applicability is separately incomplete assessment, not a conveniently smaller denominator. No applicable instance means Not applicable, not 100%. This is not an overall compliant-call certificate.

**Main reference:** S=2, F=1, U=0, N=3. Pricing is exact only after its deadline; closing is exact after a new timely reading.  
**T07B reference:** S=1, F=1, U=1, N=3; closing is attested but speech remains unverified. Manual continuation can be allowed under the synthetic rule without changing that evidence result.  
...
**Decision provenance:** A §23's evidence-based QA and uncertainty accounting; B §23's shared governed objects and possible later batch consumer. B's required state equality and mandatory scorecard are not imported.

## 24. Success metrics

### 24.1 Lead outcome — D09: improve disclosure compliance

**Primary measure: independently reviewed disclosure failures per applicable requirement opportunity, reported by requirement type.** Compare equivalent populations and rules before interpreting a reduction as improvement. The unit is a requirement instance, not an alert, reread, or model decision. Member consent and reserved-action failures are separate protected outcomes, not extra easy rows that dilute the disclosure rate.

For each known-applicable instance use three evidence outcomes: **S** = fully satisfied on time; **F** = evidenced failure; **U** = unable to verify. N = S + F + U. Report F/N with S/N and U/N alongside it; label F/N an observed evidenced-failure rate when some cases remain unverified. Show reviewed sample size, population, period, requirement/version mix, review coverage, and unknown-applicability cases separately.

**Uncertainty rule:** F/N falling while U/N rises is not sufficient evidence of better compliance. Attested closings remain U until independent evidence resolves them. Operationally permitted recovery may be counted as handled under the approved procedure, but must not be described as independently verified speech. An unverified case is not automatically advocate failure either.
...
|---|---|
| **Lead compliance outcome** | Type-specific reviewed disclosure failures, timing failures, uncertainty, and fully satisfied instances under consistent assessment. Separate timely prevention-support behavior from detection/recovery after a failure. |
...
| **Member experience** | Avoidable repetition/waiting, pressure, clarity, and appropriately collected feedback. Generated sentiment and advocate acceptance do not establish member satisfaction. |
...
**Decision provenance:** A §25's owned learning loop and local validation; B §25's shadow evaluation, explicit prototype/pilot distinction, later call-log mining and cross-channel destination. Replaces A's too-narrow initial compliance exposure and B's “never-wrong,” automatic authority-promotion, and zero-risk shadow claims.
...
| **Constraints** | A's 1/2/5/8-second targets are useful provisional prototype goals. | Measure and report against them. Field budgets require actual user and system evidence. |
| **Constraints** | One call, eight contrasts, and compact direct checks are a suitable take-home scope. | Execute before claiming completion; retain the remaining catalog as later work. |
...
|---|---|
| Productivity as the lead outcome with compliance only a safeguard | Misaligns the brief. Lead with disclosure compliance improvement, alongside service quality and measured workload/economics. |
...
This is a reflection from reviewing these definitions, not a claim that a particular architecture was built, that users were already tested, or that an unprovided incident occurred. It adopts B §30's explicit retrospective structure without inventing personal process history.
...
### 31.1 What is retained and what is changed

**Base: A.** Keep its section organization, member-centered whole-call behavior, fictional facts, matched comparison, exact selected medication scope, meaningful human submission/result distinction, specific coverage case/role gap, and truthful QA/outcome rules. Retain the main actual pricing miss and clear closing reread.

The imports are specific decisions, not B's full architecture, fixture sizes or roadmap claims.

| Change | Source / reason | Final location and consequence |
|---|---|---|
| Compliance-led business proposition and primary outcome | [H] business/problem; [B] §7's explicit compliance outcome; later priority correction | §§5/7/24/25 replace productivity-led headline, metric hierarchy and expansion criteria. No rare-event assumption or composite work/correctness ratio. |
| Clear must-build / must-prove / later handoff | [B] reading guide, corrected for its oversized tiers and optional-scorecard conflict | Header and §§18–20: two full runs, eight contrasts, compact direct checks; optional QA outside core. |
| Visible call type and appropriate current flow | [B] §§13–14; preserve [A] §13's open-needs behavior | §§9/11/13/14: automatic recognition, no extra type-confirmation click and no click-dependent obligation. |
| Five-region reference screen, source-type labels, fixed neutral nudge copy and paraphrase difference | [B] §§12/14 | §14 with [A]'s current-task attention, pending-later restraint, direct focus correction and source access. Technical traces stay subordinate. |
| Governed quick ask and explicit degraded-mode recovery | [B] §12.3 | §§12/14/19/22: same sources and authority, contextual retry/edit/status, accessible static wording and manual wrap fallback. |
| Distinct, bounded attestation option | [B] §12.1; explicit reconciled synthetic choice | §§12/15/19/23/24: closing-v2 manual continuation after an uncertain own-word attempt; remains U/ATTESTED, never consent or verified speech. Main reread and 2/3 result unchanged. |
| Three evaluation methods without exaggerated enforcement claims | [B] §18 structure plus [A] claim/evidence rigor and count-based reporting | §§18–22: exact, reference-case and human semantic review. No claim every hard failure is deterministic; fresh runs remain necessary. |
| Pilot that exercises compliance; shadow evaluation and many-channel destination | [B] §25 plus [A] learn-after-launch and local validation | §25: meaningful pricing/service-choice family, default shadow optional recommendations, actual user/review loop and explicit channels. No never-wrong, zero-risk, hidden-acceptance, or automatic-authority claims. |
| Explicit “what we would do differently” | [B] §30 format | §30, limited to supported reflection rather than invented build history. |
| Corrections reflected throughout dependent sections | This consolidation, using [H] and the review discussion | Scenarios, registry, states, test expectations, metrics, roadmap, rejected paths and this register agree. No reliance on a chat addendum. |

**Not claimed about the review:** It proposed simplifying the choice scenario, not eliminating consent controls; it proposed changing handling-metric treatment of attestation, not explicitly calling it verified speech. This contract keeps scoped authorization and separates handling from evidence without relying on those earlier mischaracterizations.

### 31.2 Humana deliverable and rubric coverage

| Humana ask | Contract content | Evidence still required |
|---|---|---|
| Problem Brief | §§2–7: primary advocate, simultaneous live-call demands, brief-derived why-now, compliance/service opportunity. | Present without fabricated research or baselines. |
...
| One tradeoff / judgment | §26: attention versus immediate visibility of useful secondary assistance. | Demonstrate deferred metformin answer and explain the accepted lost opportunity. |
...
### 31.3 Product test coverage and status
...
| Attestation versus verification | T07B and C01. |
| Changed/withdrawn authorization and unknown write | T08B, T09B, C02. |
| Exact disclosure/timing, authority, access, injection, degraded recovery, evidence/semantic truth, UI | C01–C06. |
...
|---|---|---|
| **D01** | Compliance-led whole-call assistance; accurate member service, then measured efficiency/economics. | Multiple outcomes need separate measurement; real sponsor and call evidence may change emphasis, not permit unsafe efficiency. |
...
| **D03** | One Harry call, eight short contrasts, compact boundary checks. | The sample is intentionally dense; reduce explanatory overhead, not required nudge/handoff or truthful action evidence. |
| **D04** | Exact text and governed use together; independent timing and uncertainty; limited closing-v2 attestation. | Recovery/review burden; actual approved policy may differ. |
| **D05** | Automatic routine assistance, reference five-region workspace, one current card, meaningful human decisions and direct recovery. | Some useful context is less prominent; user testing may change placement/timing. |
...
| **D09** | Lead with independently reviewed disclosure failures per applicable opportunity; always expose uncertainty and review coverage. | Requires consistent review and sufficient evidence; do not trade unknown cases for apparent improvement. |
...
| **D11** | Protect attention at the cost of immediate secondary-help visibility. | Useful optional assistance may be missed; compare compliance, user effort and member outcomes before relaxing. |
...
| **D13** | Retain A's fixed historical explanation, matched three-way comparison, metformin-only election and named pending case. Only closing recovery is versioned to v2. | More fixture detail, but one small world; no claim it is actual Humana operations. |
...
### 31.5 Remaining implementation choices and real-world dependencies
...
The final synthetic product decisions are now in this file. The next activity is implementation and evidence collection from the bounded contract—not another A-versus-B verdict or approval sequence.
